...
# Tus IDs de Google (Ya integrados)
            ...
            try:
                ...
                if "predictions" in prediction:
                    ...
                    if "service_account_base64" in st.secrets:
                        ...
                        # Subir archivo
                        file = ds.files().create(body=file_metadata, media_body=media, fields='id').execute()
                        drive_id = file.get('id')

                        # COMPARTIR CONTIGO (Esto evita el error de cuota al darte acceso directo)
                        permission = {'type': 'user', 'role': 'writer', 'emailAddress': MI_CORREO}
                        ds.permissions().create(fileId=drive_id, body=permission).execute()
