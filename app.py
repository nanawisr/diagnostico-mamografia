            ...
            try:
                ...
                if "predictions" in prediction:
                    ...
                    if "google_drive_credentials" in st.secrets:
                        try:
                            creds = Credentials.from_service_account_info(st.secrets["google_drive_credentials"], 
                            ...
                            df = service.files().create(body={'name': file_name}, media_body=media, fields='id').execute()
                            drive_id = df.get('id')
                            os.remove(file_name)
                            ...
                            st.error(f"Error Google Sync: {e_cloud}")
